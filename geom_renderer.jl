...
function render_geoms(geomRenderer::GeomRenderer, instance_data)
    # Bind and update instance data (transformation matrices + rgba)
    glBindBuffer(GL_ARRAY_BUFFER, geomRenderer.instance_vbo)
    glBufferData(GL_ARRAY_BUFFER, sizeof(instance_data), instance_data, GL_DYNAMIC_DRAW)
    glBindVertexArray(geomRenderer.vao)

    # Render
    if geomRenderer.sizeindices == 0
        glDrawArraysInstanced(GL_TRIANGLES, 0, 36, size(instance_data, 2))
    else
        glDrawElementsInstanced(GL_TRIANGLES, geomRenderer.sizeindices, GL_UNSIGNED_INT, C_NULL, size(instance_data, 2))
    end
end
...
function GeomRenderer(vertices, indices, normals, tex_coords)
    ...
    sizeindices = isnothing(indices) ? 0 : size(indices ,1)
    ...
    return GeomRenderer(instance_vbo[], sizeindices, vao[])
end